        ...
        <div id="container">
            <div id="center">
                <div id="items" v-for="(theme, index) in theme_list" :key="index">
                            <div id="item">
                                <div id="infos">
                                    <span id="info"><a :href="'/a/home/undefined/theme/' + theme.id">{{ theme.id }}</a></span>&emsp;
                                    <span id="info"><a :href="'/a/user/' + theme.user_id">{{ theme.user_id }}</a></span>&emsp;
                                    <span id="info"><a >{{ theme.category_id }}</a></span>&emsp;
                                    <span id="info"><a >{{ theme.comment_count }}</a></span>&emsp;
                                    <span id="info">{{ theme.view_count }}</span>&emsp;
                                    <span id="info"> {{ theme.created_at }} </span>
                                </div> 
                                <div id="item-title">
                                  <a :href="'/a/home/undefined/theme/' + theme.id" title="theme.title"> {{ theme.title }} </a>
                                </div>
                            </div>
                      </div>
            </div>
...
export default {
  ...
  mounted: function() {
      ...
      fetch(URLprefix + 'api/admin/themes',{
                  method: 'GET',
                  mode: 'cors'
              }).then(response => response.json())
              .then(json => {
                  this.theme_list = json.admin_themes.reverse()
              })
              .catch((e) => {
                console.log(e)
              })
